...
document.addEventListener('DOMContentLoaded', () => {
    // INDICADOR DISCRETO DE VISITA EN LA ESQUINA
    const visitDot = document.getElementById('visit-indicator');
    
    if (visitDot) {
        const hasBeenViewed = localStorage.getItem('portfolio_viewed_status');
        
        if (hasBeenViewed) {
            // Si ya se abrió antes, el punto se pone verde
            visitDot.classList.add('viewed');
            visitDot.setAttribute('title', 'Status: Viewed by recruiter/client ✔️');
        } else {
            // Si es la primera vez que se abre, se marca como visto para las siguientes veces
            localStorage.setItem('portfolio_viewed_status', 'true');
            visitDot.setAttribute('title', 'Status: Fresh visit recorded!');
            // Opcional: si quieres que cambie a verde en cuanto tú mismo lo abras y veas:
            setTimeout(() => {
                visitDot.classList.add('viewed');
            }, 1500);
        }
